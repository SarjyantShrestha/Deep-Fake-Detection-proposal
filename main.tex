\documentclass[12pt]{report}
\usepackage{setspace}
\usepackage{titlesec}
\usepackage[acronym]{glossaries}
\usepackage[utf8]{inputenc}
\usepackage[top=1in,bottom=1in,left=1.5in,right=1in,headheight=0.5in,headsep=0.5in]{geometry}
\usepackage{graphicx}
\usepackage{subcaption}
\usepackage{amsmath,amssymb,amsfonts}
\usepackage[hidelinks]{hyperref}
\usepackage{cite}
\usepackage{float}
\usepackage{array}
\usepackage{caption}
\usepackage{multirow}
\usepackage[table]{xcolor}
\usepackage{fix-cm}
\usepackage{ragged2e}
\usepackage{tabularx}

\renewcommand{\acronymname}{List of Abbreviations}
\makeglossaries
\newacronym{gans}{GANs}{\textbf{G}enerative \textbf{A}dversarial \textbf{N}etworks}
\newacronym{cnn}{CNN}{\textbf{C}onvolution \textbf{N}eural \textbf{N}etwork}
\newacronym{roc}{ROC}{\textbf{R}eciever \textbf{O}perating \textbf{C}haracteristic \textbf{C}urve}
\newacronym{auc}{AUC}{\textbf{A}rea \textbf{U}nder \textbf{T}he \acrshort*{roc} \textbf{C}urve}
\newacronym{amten}{AMTEN}{\textbf{A}daptive \textbf{M}anipulation \textbf{T}races \textbf{E}xtraction \textbf{N}etwork}
\newacronym{mcnet}{MCNet}{\textbf{M}ulti \textbf{L}evel \textbf{C}orrection \textbf{N}etwork}

% custom command for chapter headings
\titleformat{\chapter}[display]
  {\rmfamily\bfseries\fontsize{16}{18}\selectfont}{\chaptertitlename\ \thechapter}{20pt}{\fontsize{16}{18}\selectfont\bfseries}

% setting for section headings
\titleformat{\section}[block]
  {\rmfamily\bfseries\fontsize{14}{16}\selectfont}{\thesection}{1em}{}

% setting for subsection headings
\titleformat{\subsection}[block]
  {\rmfamily\bfseries\fontsize{13}{14}\selectfont}{\thesubsection}{1em}{}

% setting for subsubsection headings
\titleformat{\subsubsection}[block]
  {\rmfamily\bfseries\fontsize{12}{12}\selectfont}{\thesubsubsection}{1em}{}



% adjusting spacing before and after paragraphs
\setlength{\parindent}{0pt}
\setlength{\parskip}{6pt}

% adjusting line spacing
\onehalfspacing

\begin{document}

  \include{title}
  \pagenumbering{roman}
  \include{chapter/abstract}
  \tableofcontents
  \listoffigures
  \listoftables
  \glsaddall
  \printacronyms
  \include{chapter/chapter_1}
  \include{chapter/chapter_2}
  \include{chapter/chapter_3}
  \include{chapter/chapter_5}
  \include{chapter/chapter_6}
  \bibliographystyle{unsrt}
  \bibliography{references}
\end{document}